...
async function redirect2Pan(r) {
    ...
    const embyHost = process.env.EMBY_ADDRESS; //这里默认emby的地址是宿主机,要注意iptables给容器放行端口
    ...
    const alistApiPath = process.env.ALIST_API; //访问宿主机上5244端口的alist api, 要注意iptables给容器放行端口
    ...
    //const alistFilePath = embyRes.replace('DOCKER_ADDRESS', 'http://127.0.0.1:80').replace('http://172.17.0.1:5678', 'http://127.0.0.1:80').replace('http://xiaoya.host:5678', 'http://127.0.0.1:80');
    const alistFilePath = embyRes.replace('DOCKER_ADDRESS', alistApiPath).replace('http://172.17.0.1:5678', alistApiPath).replace('http://xiaoya.host:5678', alistApiPath);
    r.warn(`请求小雅容器: alistFilePath: ${alistFilePath}`);
